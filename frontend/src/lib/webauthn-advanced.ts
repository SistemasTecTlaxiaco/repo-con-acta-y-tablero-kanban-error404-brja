// Intentar cargar implementaciones nativas opcionales solo si están disponibles.
let WebAuthn: any = undefined;
let Capacitor: any = undefined;
try {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  WebAuthn = require('@darkedges/capacitor-native-webauthn').WebAuthn;
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  Capacitor = require('@capacitor/core').Capacitor;
} catch (e) {
  // No hay Capacitor/nativo — seguimos en modo browser
  WebAuthn = undefined;
  Capacitor = undefined;
}
...
// Convert Node Buffer or Uint8Array to a proper ArrayBuffer slice for WebAuthn
function toArrayBufferSlice(input: Uint8Array | Buffer): ArrayBuffer {
  const u8 = input instanceof Uint8Array ? input : new Uint8Array(input as any);
...
export async function isWebAuthnSupported(): Promise<boolean> {
  try {
    if (WebAuthn && typeof WebAuthn.isWebAuthnAvailable === 'function') {
      const result = await WebAuthn.isWebAuthnAvailable();
      return !!result?.value;
    }

    // Fallback browser check
    if (typeof window !== 'undefined' && window.PublicKeyCredential) return true;
    return false;
  } catch (error) {
...
export async function isWindowsHelloAvailable(): Promise<boolean> {
  // If running in native with Capacitor we could attempt a native-specific check.
  if (Capacitor && WebAuthn && typeof WebAuthn.isWebAuthnAvailable === 'function') {
    return await isWebAuthnSupported();
  }
  // In browser, detect if platform authenticator likely available (rudimentary):
...
export async function createPasskey(
  ...
  try {
    ...
    // If native WebAuthn helper is available, prefer it. Otherwise, fallback to browser
    if (WebAuthn && typeof WebAuthn.startRegistration === 'function') {
      const rpId = Capacitor && typeof Capacitor.isNativePlatform === 'function' && Capacitor.isNativePlatform()
      ...
      const registerRes = await WebAuthn.startRegistration({
        ...
        authenticatorSelection: {
          ...
          residentKey: Capacitor && Capacitor.getPlatform && Capacitor.getPlatform() === 'android' ? 'preferred' : 'discouraged',
    ...
    // Fallback: use browser WebAuthn via navigator.credentials.create
    // Build options similar to web implementation in webauthn.ts
    const challengeBuf = base64url.toBuffer('createchallenge');
    const userIdBuf = base64url.toBuffer(username);
    const opts: PublicKeyCredentialCreationOptions = {
      challenge: toArrayBufferSlice(challengeBuf),
      rp: { name: 'SoroPass' },
      user: { id: toArrayBufferSlice(userIdBuf), name: username, displayName: userDisplayName || username },
      pubKeyCredParams: [{ alg: -7, type: 'public-key' }],
  ...
  // Ensure browser supports credentials
  const credential = (await navigator.credentials.create({ publicKey: opts as any })) as unknown;
  return credential as unknown as WebAuthnRegistrationResult;
  ...
  try {
    ...
    if (WebAuthn && typeof WebAuthn.startAuthentication === 'function') {
      const signRes = await WebAuthn.startAuthentication({
        challenge: base64url('createchallenge'),
        rpId: Capacitor && typeof Capacitor.isNativePlatform === 'function' && Capacitor.isNativePlatform() ? 'passkey.sorobanbyexample.org' : undefined,
        userVerification: 'discouraged',
      });
      console.log('Authentication result (native):', JSON.stringify(signRes, null, 2));
      return signRes as WebAuthnAuthenticationResult;
    }

    // Fallback to browser navigator.credentials.get
    const challengeBuf = base64url.toBuffer('createchallenge');
    ...
    const assertion = (await navigator.credentials.get({ publicKey: opts as any })) as unknown;
    console.log('Authentication result (browser):', assertion);
    return assertion as unknown as WebAuthnAuthenticationResult;
